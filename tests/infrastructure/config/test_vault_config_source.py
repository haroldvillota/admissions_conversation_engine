...
def test_vault_config_source_loads_prefixed_and_unprefixed_secret_names(monkeypatch) -> None:
    monkeypatch.setenv("AZURE_KEY_VAULT_URL", "https://example.vault.azure.net/")

    fake_client = FakeSecretClient(
        {
            "secret--myapp--rag--vector-store--dsn": "postgresql://from-vault",
            "llm--default--api-key": "openai-key-from-unprefixed-name",
            "secret--myapp--observability--public-key": "pk-test",
        }
    )
    ...
    assert result == {
        ...
        "LLM__DEFAULT__API_KEY": "openai-key-from-unprefixed-name",
